...
function Form(props) {
    ...
    useEffect(() => {
        ...
        const charsLimit = countChars <= 4096
        let haveGames = false
        ...
        setIsSubmitDisabled(isCityFilled && isStateFilled && haveGames && charsLimit && descriptionValidation.length === 0)

    }, [userData.city, userData.state, userData.boardgames, userData.auctions, isStandard, countChars])

    useEffect(() => {
        setUserData({...userData, boardgames: boardgames})
        const externalAuction = boardgames.filter(item => item.type === "Leilão Externo")
    ...
    useEffect(() => {
        let chars = userData.city.length + userData.state.length + userData.general_description.length
        ...
        if (isStandard) {
            ...
            for (let game of auctions) {
                chars += game.name.length + game.starting_price.length + game.increment.length
            }
        }
    ...
    return (
        ...
        >
            <ToggleButtonGroup
                ...
                sx={{ mt: 3, mb: 1 }}
            >
                <ToggleButton value={ true } aria-label="standard">Anúncio</ToggleButton>
                <ToggleButton value={ false } aria-label="auction">Leilão</ToggleButton>  
            </ToggleButtonGroup>
            ...
            >
                        ...
                        }
                        {
                            submitStatus.message.length > 0 
                                ?   <Grid container direction="row" justifyContent="center" alignItems="center">
                                        <Alert severity="error" sx={{ mt: 3 }}>{ submitStatus.message }</Alert>
                                    </Grid>
                                :   <></>
                        }
                    </DialogContent>
                <DialogActions>
                    ...
                    <Button onClick={ event => onSubmit(event) } disabled={ submitStatus.loading } autoFocus>
                        Enviar
                    </Button>
                </DialogActions>
            </Dialog>
