...
function ItemForm(props) {
  ...
  const formatPrice = (value) =>
    parseFloat(value.replace(",", ".")).toLocaleString("pt-br", {
      maximumFractionDigits: 2,
      minimumFractionDigits: 2,
    });

  const comparaJogosThrottleFetch = useMemo(
    () =>
      throttle((name) => {
  ...
  const onItemSubmit = (event) => {
    ...
    if (props.isStandard && doesBoardgameExists) {
      const formattedPrice =
        price === "" ? formatPrice("100.00") : formatPrice(price);
      ...
      const newItem = {
        ...
        increment:
          increment === "" ? formatPrice("5.00") : formatPrice(increment),
      };
  ...
  useEffect(() => {
    ...
    if (props.isStandard) {
      ...
      const isStartingPriceFilled =
        startingPrice !== "" && !startingPrice.match(priceValidator);
      const isIncrementOk =
        !increment.match(priceValidator) || parseFloat(increment);
  ...
  return (
    <>
      ...
      >
        {props.isStandard ? (
          <FormControl variant="standard" fullWidth sx={{ width: 85 + "%" }}>
            <Select
              labelId="type-option"
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                ...
                label="Jogo"
                onChange={onBoardgameChange}
                error={boardgameError.length > 0 ? true : false}
        ...
        {props.isStandard ? (
          ...
          <Grid container spacing={1} sx={{ width: 86 + "%" }}>
            <Grid item xs={8}>
              <FormControl variant="standard" fullWidth>
                <TextField
                  ...
                  sx={{ mt: 1 }}
                  onChange={(e) =>
                    priceValidation(e.target.value, "startingPrice")
                  }
                  value={startingPrice}
                />
              </FormControl>
